 ...
 * ============================================================================
 * $Author: 孤独三少 $
 * $Id: group_buy.php 17217 2011-01-19 06:29:08Z 孤独三少 $
 */
...
elseif ($_REQUEST['act'] =='insert_update')
{
    ...
    if (isset($_FILES['act_thumb']['error'])) // php 4.2 版本才支持 error
    {
        // 最大上传文件大小
        ...
        // 商品图片
        if ($_FILES['act_thumb']['error'] == 0)
        {

            if (!$image->check_img_type($_FILES['act_thumb']['type']))
            {
                sys_msg($_LANG['invalid_act_thumb'], 1, array(), false);
            }
        }
        elseif ($_FILES['act_thumb']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['act_thumb_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['act_thumb']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['act_thumb_too_big'], $htm_maxsize), 1, array(), false);
        }

		if($_FILES['act_thumb']['tmp_name'] != '' && $_FILES['act_thumb']['tmp_name'] != 'none')
		{
			 /* 创建当月目录 */
			$dir = date('Ym');
			$act_thumb   = $image->upload_image($_FILES['act_thumb'],'groupimg/'.$dir); // 原始图片
		}else{
			$act_thumb = $_POST['act_thumbh'];
		}
    ...
    else
    {
        ...
        $group_buy = array(
			...
			'group_subtitle'   => $_POST['group_subtitle'],
			'group_address'   => $_POST['group_address'],
			'group_decorate'   => $_POST['group_decorate'],
			'group_early'   => $_POST['group_early'],
			'group_property_time'   => $_POST['group_property_time'],
			'group_property'   => $_POST['group_property'],
			'group_making_room'   => $_POST['group_making_room'],
			'group_sum_house'   => $_POST['group_sum_house'],
            'act_desc'   => $_POST['act_desc'],
