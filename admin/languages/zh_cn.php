<?php



$_LANG['colon'] = '：';

$_LANG['close'] = '关闭';

$_LANG['cp_home'] = $_CFG['shop_name'].'管理面板';

$_LANG['admin_panel'] = '管理面板';

$_LANG['forget_pwd'] = '忘记密码?';

$_LANG['label_username'] = '管理员帐号：';

$_LANG['signin_now'] = '登入后台';

$_LANG['remember'] = '记住我';

$_LANG['user_name'] = '管理员帐号：';

$_LANG['email'] = 'E-mail：';



/* 导航栏 */

$_LANG['02_cat_and_goods'] = '户型和分类';

$_LANG['03_promotion'] = '促销';

$_LANG['04_order'] = '订单';

$_LANG['05_banner'] = '广告';

$_LANG['06_stats'] = '统计';

$_LANG['07_content'] = '文章';

$_LANG['08_members'] = '会员';

$_LANG['09_others'] = '杂项';

$_LANG['10_priv_admin'] = '权限';

$_LANG['11_system'] = '系统';

$_LANG['12_template'] = '模板';

$_LANG['13_backup'] = '数据库';

$_LANG['14_sms'] = '短信';

$_LANG['15_rec'] = '推荐';

$_LANG['16_email_manage'] = '邮件群发';



$_LANG['shop_guide'] = '开店向导';

$_LANG['set_navigator'] = '设置导航栏';

$_LANG['about'] = '关于 ECSHOP';

$_LANG['preview'] = '查看网站';

$_LANG['menu'] = '菜单';

$_LANG['help'] = '帮助';

$_LANG['signout'] = '退出';

$_LANG['profile'] = '个人设置';

$_LANG['view_message'] = '管理员留言';

$_LANG['send_msg'] = '发送留言';

$_LANG['calculator'] = '计算器';

$_LANG['clear_cache'] = '清除缓存';



$_LANG['self_nav'] = '个人导航';

$_LANG['edit_self_nav'] = '编辑个人导航';





$_LANG['mailsend_fail'] = '邮件 %s 发送失败!';

$_LANG['mailsend_ok'] = '邮件 %s 发送成功!还有 %s 邮件未发送!';

$_LANG['mailsend_finished'] = '邮件 %s 发送成功!全部邮件发送完成!';

$_LANG['mailsend_null'] = '邮件发送列表空!';

$_LANG['mailsend_skip'] = '继续发送下一条...';

$_LANG['email_sending'] = '正在处理邮件发送队列...';

$_LANG['pause'] = '暂停';

$_LANG['conti'] = '继续';

$_LANG['str'] = '已经发送了 %d 封邮件。';