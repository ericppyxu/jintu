 ...
 * ============================================================================
 * $Author: 孤独三少 $
 * $Id: lib_main.php 17217 2011-01-19 06:29:08Z 孤独三少 $
*/
...
function create_html_editor($input_name, $input_value = '',$fckid=0) 
{
	...
	if ($fckid)
	{
	$smarty->assign('FCKeditor'.$fckid, $kindeditor);
	}
	else
	{
	$smarty->assign('FCKeditor', $kindeditor);
 ...
 */
//function create_html_editor($input_name, $input_value = '')
function create_html_editor2($input_name, $output_name ,  $input_value = '',$fckid=0) 
{
    global $smarty;
	...
	if ($fckid)
	{
	$smarty->assign($output_name.$fckid, $kindeditor);
	}
	else
	{
	$smarty->assign($output_name,  $kindeditor);
	} 
}

/**
 * 取得房型列表：用于把房型添加到组合、关联类、赠品类
