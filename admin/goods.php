 ...
 * ============================================================================
 * $Author: 孤独三少 $
 * $Id: goods.php 17217 2011-01-19 06:29:08Z 孤独三少 $
*/
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    else
    {
        ...
        $link_goods_list    = get_linked_goods($goods['goods_id']); // 关联房型
		...
		/* 户型列表 */
        $sql = "SELECT * FROM " . $ecs->table('door_model') . " WHERE goods_id = '$goods[goods_id]'";
        $door_model_list = $db->getAll($sql);
	...
	$smarty->assign('door_model_list', $door_model_list);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        // 如果系统支持GD，缩放房型图片，且给房型图片和相册图片加水印
	...
	$goods_decorate = $_POST['goods_decorate'];
	$goods_early = $_POST['goods_early'];
	$goods_property_time = $_POST['goods_property_time'];
	$goods_property = $_POST['goods_property'];
	$goods_making_room = $_POST['goods_making_room'];
	$goods_sum_house = $_POST['goods_sum_house'];
	$goods_mobile_phone = $_POST['goods_mobile_phone'];
	$goods_area = $_POST['goods_area'];
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, goods_desc2, goods_desc3, add_time, last_update, goods_type, rank_integral, suppliers_id, goods_address, goods_decorate, goods_early, goods_property_time, goods_property, goods_making_room, goods_sum_house, goods_mobile_phone, goods_area)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', " . " '$_POST[goods_desc2]', " ." '$_POST[goods_desc3]', '" .gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$goods_address', '$goods_decorate', '$goods_early', '$goods_property_time', '$goods_property', '$goods_making_room', '$goods_sum_house', '$goods_mobile_phone', '$goods_area')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, goods_desc2, goods_desc3, add_time, last_update, goods_type, extension_code, rank_integral, goods_address, goods_decorate, goods_early, goods_property_time, goods_property, goods_making_room, goods_sum_house, goods_mobile_phone, goods_area)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', " ." '$_POST[goods_desc2]', " ." '$_POST[goods_desc3]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$goods_address', '$goods_decorate', '$goods_early', '$goods_property_time', '$goods_property', '$goods_making_room', '$goods_sum_house', '$goods_mobile_phone', '$goods_area')";
        }
    }
    else
    {
	...
	/*添加户型介绍*/
	/* 检查图片：如果有错误，检查尺寸是否超过最大值；否则，检查文件类型 */
    if (isset($_FILES['door_img']['error'])) // php 4.2 版本才支持 error
    {
        // 最大上传文件大小
        $php_maxsize = ini_get('upload_max_filesize');
        $htm_maxsize = '2M';
        ...
        // 商品图片
        if ($_FILES['door_img']['error'] == 0)
        {

            if (!$image->check_img_type($_FILES['door_img']['type']))
            {
                sys_msg($_LANG['invalid_act_thumb'], 1, array(), false);
            }
        }
        elseif ($_FILES['door_img']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['act_thumb_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['door_img']['error'] == 2)
        {
            sys_msg(sprintf($_LANG['act_thumb_too_big'], $htm_maxsize), 1, array(), false);
        }

		if($_FILES['door_img']['tmp_name'] != '' && $_FILES['door_img']['tmp_name'] != 'none')
		{
			 /* 创建当月目录 */
			$dir = date('Ym');
			$door_img   = $image->upload_image($_FILES['door_img'],'doorimg/'.$dir); // 原始图片
		}

	}
	$door_title = $_POST['door_title'];
	$unscramble = $_POST['unscramble'];
	$door_note = $_POST['door_note'];
	if($door_title && $unscramble && $door_note)
	{
	   $sql = "INSERT INTO  " .$ecs->table('door_model'). "(`door_model_id` ,`goods_id` ,`img` ,`title` ,`unscramble` ,`note`)VALUES (NULL ,  '$goods_id',  '$door_img',  '$door_title',  '$unscramble',  '$door_note')";
	   $db->query($sql);
	}
...
//-- 删除户型介绍
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_door_model')
{
    check_authz_json('goods_manage');
    ...
    /* 删除图片文件 */
    $sql = "SELECT img " .
            " FROM " . $GLOBALS['ecs']->table('door_model') .
            " WHERE door_model_id = '$img_id'";
    $row_img = $GLOBALS['db']->getOne($sql);
    ...
    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('door_model') . " WHERE door_model_id = '$img_id' LIMIT 1";
    $GLOBALS['db']->query($sql);
 ...
 * @param   string  $extension_code 虚拟房型扩展代码，实体房型为空
...
/**
 * 修改房型库存
 * @param   string  $goods_id   房型编号，可以为多个，用 ',' 隔开
