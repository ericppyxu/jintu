...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        $goods_attr[] = explode(' ', trim($row['goods_attr'])); //将房型属性拆分为一个数组
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
		...
		if ($_CFG['ecsdxt_order_shipped'] == '1' && $row['mobile_phone'] != '')
		{
			...
			$content = "{\"code\":\"".$order['order_sn']."\",\"product\":\"惠居信息科技\",\"item\":\"惠居信息科技\"}";

			$ret = sendsms($row['mobile_phone'], $content ,$_CFG['ecsdxt_order_shipped_value']);
		}
    }
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('consignee' == $step)
    {
        ...
        elseif (isset($_POST['finish']))
        {
            /* 如果是编辑且存在实体房型，检查收货人地区的改变是否影响原来选的配送 */
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('prepare' == $operation)
    {
		...
		if ($_CFG['ecsdxt_order_picking'] == '1' && $row['mobile_phone'] != '')
		{
			...
			$ret = sendsms($row['mobile_phone'], $content ,$_CFG['ecsdxt_order_picking_value']);
		}
    }
    /* 分单确认 */
    elseif ('split' == $operation)
    {
        ...
        foreach ($goods_list as $key => $value)
        {
            ...
            if ($value['extension_code'] == 'package_buy')
            {
                foreach ($value['package_goods_list'] as $pg_key => $pg_value)
                {
                    ...
                    /* 房型（超值礼包） 虚拟房型列表 package_virtual_goods*/
        ...
        if (true)
        {
            ...
            /* 更新订单的非虚拟房型信息 即：房型（实货）（货品）、房型（超值礼包）*/
...
/**
 * 取得礼包列表
 * @param   integer     $package_id  订单房型表礼包类房型id
 * @return array
 */
 ...
 * 更新订单房型信息
 * @param   int     $order_id       订单 id
 * @param   array   $_sended        Array(‘房型id’ => ‘此单发货数量’)
        ...
        if (is_array($value))
        {
            ...
            foreach ($goods_list as $goods)
            {
                ...
                foreach ($goods['package_goods_list'] as $pg_key => $pg_value)
                {
                    ...
                    {
                        $pg_is_end = false; // 此超值礼包，此房型未全部发货
 ...
 * @param   int     $order_id       订单 id
 * @param   array   $_sended        Array(‘房型id’ => ‘此单发货数量’)
 * @param   array   $virtual_goods  虚拟房型列表
 * @return  Bool
 */
...
/**
 *  超值礼包虚拟卡发货、跳过修改订单房型发货数的虚拟卡发货
 *
 * @access  public
 * @param   array      $goods      超值礼包虚拟房型列表数组
