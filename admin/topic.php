 ...
 * ============================================================================
 * $Author: 孤独三少 $
 * $Id: topic.php 17217 2011-01-19 06:29:08Z 孤独三少 $
*/
...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if (!$isadd)
    {
		...
		create_html_editor('topic_intro2', $topic['intro2'],2); 
    ...
    else
    {
		...
		create_html_editor('topic_intro2');
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$topic_phone= $_POST['topic_phone'];
	$topic_phone_extension= $_POST['topic_phone_extension'];
	$is_check  = empty($_POST['is_check']) ? 0 : intval($_POST['is_check']);
	$hits  = empty($_POST['hits']) ? 0 : intval($_POST['hits']);
	$show_num  = empty($_POST['show_num']) ? 0 : intval($_POST['show_num']);
	$call_volume  = empty($_POST['call_volume']) ? 0 : intval($_POST['call_volume']);
	

    if ($is_insert)
    {
        $sql = "INSERT INTO " . $ecs->table('topic') . " (title,start_time,end_time,data,intro,template,css,topic_img,title_pic,base_style, htmls,keywords,description,phone,phone_extension,intro2,is_check,hits,show_num,call_volume)" .
                "VALUES ('$_POST[topic_name]','$start_time','$end_time','$data','$_POST[topic_intro]','$_POST[topic_template_file]','$_POST[topic_css]', '$topic_img', '$title_pic', '$base_style', '$htmls','$keywords','$description','$topic_phone','$topic_phone_extension','$_POST[topic_intro2]','$is_check','$hits','$show_num','$call_volume')";
    }
    else
    {
        $sql = "UPDATE " . $ecs->table('topic') .
                "SET title='$_POST[topic_name]',start_time='$start_time',end_time='$end_time',data='$data',intro='$_POST[topic_intro]',template='$_POST[topic_template_file]',css='$_POST[topic_css]', topic_img='$topic_img', title_pic='$title_pic', base_style='$base_style', htmls='$htmls', keywords='$keywords', description='$description', phone='$topic_phone', phone_extension='$topic_phone_extension',intro2='$_POST[topic_intro2]', is_check='$is_check', hits='$hits', show_num='$show_num' , call_volume='$call_volume' " .
