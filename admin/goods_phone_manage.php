...
if ($_REQUEST['act'] == 'list')
{
	...
	//	/* 显示页面 */
    assign_query_info();
    $smarty->display('goods_phone_list.htm');
   //函数如下，仅作为参考
...
}elseif ($_REQUEST['act'] == 'query')
{
	...
	make_json_result($smarty->fetch('goods_phone_list.htm'), '', array('filter' => $card_list['filter'], 'page_count' => $card_list['page_count']));
}elseif($_REQUEST['act'] == 'remove')
{
	$id   = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : '';
	if($id)
	{
	  $sql = "DELETE FROM ".$GLOBALS['ecs']->table('subscribe_housse') ." WHERE `subscribe_housse_id` = '$id'";
	  $db->query($sql);
	  header("Location: goods_phone_manage.php"); 
	  exit;
	}
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT * FROM " .$ecs->table('subscribe_housse').
           " WHERE subscribe_housse_id = '".$_REQUEST['id']."'";
    $user_info = $db->getRow($sql);


    /* 模板赋值 */
    $smarty->assign('ur_here',     "预约用户管理");
    $smarty->assign('action_link', array('text' => $_LANG['admin_list'], 'href'=>'goods_phone_manage.php?act=list'));
    ...
    assign_query_info();
    $smarty->display('goods_phone_info.htm');
}
/*------------------------------------------------------ */
//-- 更新预约信息
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'update')
{
    ...
    /* 变量初始化 */
    $subscribe_housse_id    = !empty($_POST['id'])        ? intval($_POST['id'])   : 0;
	...
	{
	  //更新预约信息
	  $sql = "UPDATE " .$ecs->table('subscribe_housse'). " SET ".
			 "reply_time = '$reply_time',note = '$note', is_check = '$is_check' ".
			 " WHERE subscribe_housse_id = '$subscribe_housse_id'";
	 //var_dump($sql);exit;
	 $db->query($sql);
   }
   /* 记录管理员操作 */
   //admin_log($_POST['user_name'], 'edit', 'admin_role_manage');
    ...
    $g_link ="goods_phone_manage.php?act=list";
    $msg = "信息更改成功！";
...
function card_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('subscribe_housse') ."WHERE subscribe_housse_id > 0" . $where;
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);
 
        $filter = page_and_size($filter);
 
        /* 查询记录 */
        $sql = "SELECT *"."FROM ". $GLOBALS['ecs']->table('subscribe_housse') .' WHERE subscribe_housse_id > 0 '. $where.' order by subscribe_housse_id ASC LIMIT ' . $filter['start'] . ',' . $filter['page_size'];
	...
	foreach($all as $key=>$val)
    {
		...
		$all[$key]['goods_name'] = $GLOBALS['db']->getOne("SELECT goods_name FROM". $GLOBALS['ecs']->table('goods') ."WHERE goods_id =".$val['goods_id']);
		if($val['reply_time'])
...
{
	$sql = "SELECT goods_id , goods_name FROM ". $GLOBALS['ecs']->table('subscribe_housse') ;
